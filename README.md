...
このリポジトリには、現時点では計画用の成果物と、実装開始前のモジュラーモノリス（modular monolith）ひな形が含まれています。`docs/planning/WORKFLOW.md` に従い、要求仕様、用語、業務ルール、受け入れ基準、データ設計、アーキテクチャ設計、Lean 仕様化計画を作成しています。

Rust 側は `apps/laborlens-rust` を単一のアプリケーション兼 workspace の入口にし、複数の独立 crate ではなく、bounded context ごとの module で整理します。現時点の Rust ひな形は本格実装ではなく、Radomil がエンジン実装を開始するための責務メモと module 境界です。
...
| ローカル UI | ローカルサーバーに接続し、実行指示、進捗確認、結果閲覧を行う。コアロジックは再実装しない |
