...
| 項目 | 採用方針 |
| --- | --- |
| 実行形態 | 利用者 PC または社内端末上のローカル実行。最終配布は Tauri desktop app とする |
| 配布形態 | Tauri installer 付き desktop app。PostgreSQL、Ollama、初期モデル `qwen3:8b` を同梱する方針とする。初期開発時は browser + local server 起動を許容する |
| UI | ローカルサーバー API を呼び出す操作・閲覧層 |
| UI 実装スタック | Tauri + Vite + React + TypeScript、shadcn/ui、TanStack Table、TanStack Query、assistant-ui |
| ローカルサーバー | アプリケーション境界、API、RunId 発行、ジョブ登録、結果提供 |
| ローカル DB | installer 同梱の PostgreSQL を managed local cluster として使う |
| ジョブキュー | PostgreSQL backed job queue |
| バックグラウンド処理 | ローカルワーカーで実行 |
| ガイド AI | installer 同梱の Ollama 上の `qwen3:8b` を初期モデルとする |
| ガイド AI 接続 | UI から Ollama を直接呼ばず、Local Server API が許可済み文脈だけを渡す |
...
| コンポーネント | 概要 |
| --- | --- |
| UI | React による利用者操作、進捗確認、結果閲覧、再確認指示を行う画面層。 |
| Tauri Desktop Shell | installer 付き desktop app、React UI の表示、file dialog、アプリ設定、local server / worker 起動管理を担う shell。 |
...
| Tauri Desktop Shell | React UI の desktop app 化、installer 配布、file dialog、アプリ設定、local server / worker の起動管理、OS 通知 | CSV 検査、集計、プライバシー抑制、DB 直接更新、Ollama への抑制前データ送信、業務判断 |
...
| ADR-ARCH-009 | UI 技術スタック採用 | local UI は Tauri + Vite + React + TypeScript、shadcn/ui、TanStack Table、TanStack Query、assistant-ui で実装する。AI assistant は UI から Ollama を直接呼ばず、Local Server API 経由で接続する。 |
| ADR-ARCH-010 | Tauri installer 配布採用 | 本番配布は Tauri installer 付き desktop app とする。Tauri は配布、起動管理、file dialog、アプリ設定を担当し、業務ロジック、DB 書き込み、AI 安全境界は Local Server API / worker 側に置く。 |
| ADR-ARCH-011 | PostgreSQL / Ollama 同梱配布採用 | 本番配布では PostgreSQL、Ollama、初期モデル `qwen3:8b` を installer に同梱する方針とする。利用者の手動インストールを避け、Tauri app が managed local services として起動確認、初期化、停止、更新確認を扱う。 |
...
初期 data-ingest slice は Rust modular monolith の `ingest` bounded context に置き、Rust の `csv` package による狭い CSV pipeline から開始する。目的は高速な DataFrame 集計ではなく、汚れた CSV を行単位で読み、標準列名解決、型変換、検査、issue 化を安定させることである。Polars は後段の分析・高速集計候補とする。
...
| ARCH-DECISION-MODMONO-001 | Rust 側の初期本番構成は `apps/laborlens-rust` の modular monolith とし、ingest、workforce analysis、privacy/safety、reporting、guidance を bounded context module として整理する。 | `ARCHITECTURE.md`, `REPOSITORY-PLAN.md` |
| ARCH-DECISION-REPORT-001 | Rust monolith は JSON、CSV、Markdown、artifact manifest までを生成し、PDF は Python などの thin renderer に残す。 | `DATA-DESIGN.md`, `REPOSITORY-PLAN.md` |
