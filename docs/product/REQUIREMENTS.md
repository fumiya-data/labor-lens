...
| 疲労関連データ | 部署単位または店舗単位の負荷傾向確認 | 個人値を直接表示してはならない |
...
| ID | 要求 |
| --- | --- |
| FR-SCHEMA-001 | LaborLens は、データ種別ごとの必須列、任意列、列名変更候補を確認できなければならない。 |
| FR-SCHEMA-002 | LaborLens は、日付、時刻、数値、ID、部署、店舗などの列形式を確認できなければならない。 |
| FR-SCHEMA-003 | LaborLens は、CSV 仕様変更の影響として、列名変更、必須列欠落、形式変更を整理できなければならない。 |
...
| FR-PRIVACY-006 | 少人数部署とみなす閾値は `BUSINESS-RULES.md` で定義し、実行設定として記録しなければならない。 |
...
| NFR-SEC-004 | アクセス制御、認証、保存時暗号化、ログマスキングの詳細は、`ARCHITECTURE.md` または `OPERATIONS.md` で定義する。 |
...
| AC-SEED-004 | FR-PRIVACY-002, SAFETY-006 | 少人数部署の集計が非表示、伏せ字、または抑制済みになること |
...
| AC-SEED-010 | AI-QUALITY-001, AI-SAFE-001..006 | ガイド AI が根拠文書付きで回答し、安全境界を越えた回答をしないこと |
...
| ID | 未決事項 | 決める理由 | 推奨される決定先 |
| --- | --- | --- | --- |
| OPEN-001 | 少人数部署とみなす人数の閾値 | プライバシー抑制の対象範囲が変わるため | `BUSINESS-RULES.md` |
| OPEN-002 | 長時間労働確認で使う初期閾値 | 労務確認 issue の出方が変わるため | `BUSINESS-RULES.md` |
| OPEN-003 | 有給取得状況で表示する集計粒度 | レポートの見せ方と集計単位が変わるため | `BUSINESS-RULES.md`, `EXTERNAL-DESIGN.md` |
| OPEN-004 | 店舗別、部署別、雇用区分別の優先表示順 | 画面やレポートで利用者が最初に見る情報が変わるため | `EXTERNAL-DESIGN.md` |
| OPEN-005 | issue code の体系 | テスト、画面、CSV 出力、Lean 仕様との対応に必要なため | `BUSINESS-RULES.md`, `ACCEPTANCE-CRITERIA.md` |
| OPEN-006 | CSV 列名の標準形と別名許容範囲 | CSV 仕様変更確認と正規化に必要なため | `DATA-DESIGN.md` |
| OPEN-007 | 10000 人 × 3 年分データの性能目標値 | 性能テストとアーキテクチャ判断に必要なため | `ARCHITECTURE.md`, `TEST-PLAN.md` |
| OPEN-008 | Ollama で採用する初期モデル | 応答品質、速度、メモリ使用量、利用者 PC の性能要件が変わるため | `ARCHITECTURE.md`, `TEST-PLAN.md` |
| OPEN-009 | RAG の検索対象文書とインデックス更新条件 | ガイド AI がどの情報を根拠として回答できるかが変わるため | `DATA-DESIGN.md`, `ARCHITECTURE.md` |
| OPEN-010 | ガイド AI の評価データセット | 日本語・英語回答品質と安全境界を検証するため | `TEST-PLAN.md` |
| OPEN-011 | 抑制前データへのアクセス制御 | 内部データと公開用出力の境界を運用で守るため | `ARCHITECTURE.md`, `OPERATIONS.md` |
| OPEN-012 | レポート出力形式 | Markdown、JSON、CSV、画面表示、PDF などの範囲が設計に影響するため | `EXTERNAL-DESIGN.md`, `DATA-DESIGN.md` |
